...
def test(user_id):
    ...
    LOG_FILE_PATH = '/home/yinjiaoyang/code/Live-Video-Streaming-Challenge-master-tecent/code_debug/test_code/log/'
    ...
    network_trace_dir = '/home/yinjiaoyang/code/Live-Video-Streaming-Challenge-master-tecent/dataset/network_trace/' + NETWORK_TRACE + '/'
    video_trace_prefix = '/home/yinjiaoyang/code/Live-Video-Streaming-Challenge-master-tecent/dataset/video_trace/' + VIDEO_TRACE + '/new_frame_trace_'
    ...
    BIT_RATE = [500.0, 850.0, 1200.0, 1850.0] #[500.0, 850.0, 1200.0, 1850.0]  # kpbs
    ...
    log_rate = open(
        '/home/yinjiaoyang/code/Live-Video-Streaming-Challenge-master-tecent/code_debug/test_code/reward_' + method + '_' + NETWORK_TRACE + '_bitratevstime_trace' + str(
            net_env.get_trace_id()), 'wb')
    log_rebuf = open(
        '/home/yinjiaoyang/code/Live-Video-Streaming-Challenge-master-tecent/code_debug/test_code/reward_' + method + '_' + NETWORK_TRACE + '_rebufvstime_trace' + str(
            net_env.get_trace_id()), 'wb')
    log_rebuf2 = open(
        '/home/yinjiaoyang/code/Live-Video-Streaming-Challenge-master-tecent/code_debug/test_code/reward_' + method + '_' + NETWORK_TRACE + '_rebufvsgop_trace' + str(
            net_env.get_trace_id()), 'wb')
    log_reward = open(
        '/home/yinjiaoyang/code/Live-Video-Streaming-Challenge-master-tecent/code_debug/test_code/reward_' + method + '_' + NETWORK_TRACE + '_rewardvsgop_trace' + str(
            net_env.get_trace_id()), 'wb')
    ...
    while True:
        ...
        if S_time_interval[0] is 0 and S_send_data_size[0] is 0:
        ...
        if time_interval is not 0 and send_data_size is not 0:
            #print(S_time_interval)
            #print(S_send_data_size)
        ...
        if decision_flag or end_of_video:
            ...
            if np.sum(S_rebuf[-51:-1]) >= 1 and REBUF_PENALTY <= 7.0:
                REBUF_PENALTY += 0.1
            ...
            log_reward.flush()
            log_rebuf.write(str(time) + '\t' + str(np.sum(S_rebuf[-51:-1])/50) + '\n')
            log_rebuf.flush()
            log_rebuf2.write(str(cnt) + '\t' + str(np.sum(S_rebuf[-51:-1])/50) + '\n')
            log_rebuf2.flush()
            reward_gop = 0
