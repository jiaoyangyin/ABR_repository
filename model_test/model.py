...
def train(x_valid):
    ...
    for i in range(40):
        ...
        if i == 0:
            ...
            x_valid = np.c_[x_valid, valid_normal]
    """
    x_validmax, x_validmin = max(x_valid), min(x_valid)
    x_valid = [x - y for x in x_valid for y in [x_validmin]]
    x_valid = [x / y for x in x_valid for y in [x_validmax - x_validmin]]
    ...
    with tf.Session() as sess:
        ...
        prediction_result = sess.run(y, feed_dict={x: np.reshape(x_valid, (BATCH_SIZE, in_width, in_channels)),
                                                   is_train: False})
