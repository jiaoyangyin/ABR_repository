...
def agent(agent_id, all_cooked_time, all_cooked_bw, net_params_queue, exp_queue):
    ...
    with tf.Session() as sess, open(LOG_FILE + '_agent_' + str(agent_id), 'wb') as log_file:
        ...
        actor.set_network_params(actor_net_params)                    #this part is only initial the network parameters, they will be updated in the loop
...
def main():
    ...
    assert len(VIDEO_BIT_RATE) == A_DIM  #if true get 1 else get AssertionError
