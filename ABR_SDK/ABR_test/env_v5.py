...
class Environment:
    def __init__(self, all_cooked_time, all_cooked_bw, random_seed=RANDOM_SEED,
        ...
        if Debug:
            ...
            self.log_file = open(logfile_path, 'w')# + "log." + current_time, "w")
        ...
        self.newest_frame = 0
        self.next_I_frame = 30

        self.video_chunk_counter = 0 #frame counter
        self.buffer_size = 0

        # pick a random trace file
        self.trace_idx = 0#np.random.randint(len(self.all_cooked_bw))
        self.cooked_time = self.all_cooked_time[self.trace_idx]
        self.cooked_bw = [x+y for x in self.all_cooked_bw[self.trace_idx] for y in [2]]
        # randomize the start point of the trace
        # note: trace file starts with time 0
        #self.mahimahi_ptr = np.random.randint(1, len(self.cooked_bw))
        #self.mahimahi_ptr = 1
        self.decision = False
        self.indicate_flag = False
        self.buffer_status = True  # indicates whether the buffer is buffering, if it is True, the client will not play videos and will buffer frames
        ...
        for bitrate in range(BITRATE_LEVELS):
            ...
            with open(self.video_size_file + str(bitrate)) as f:
                for line in f:
                    ...
                    self.video_size[bitrate].append(float(line.split()[1]))  # the size of each frame
                    self.gop_time_len[bitrate].append(float(1 / FPS))  # the duration of each frame 0.33s
                    self.gop_flag[bitrate].append(int(float(
                        line.split()[2])))  # indicate whether the frame is I frame - 0 is P frame and 1 is I frame
                    self.cdn_arrive_time[bitrate].append(
                        float(line.split()[0]))  # the time when the client can get the frame from the cdn
                    cnt += 1
    ...
    def get_video_frame(self, quality, target_buffer, latency_limit):
        ...
        if self.time < self.cdn_arrive_time[quality][
            self.video_chunk_counter] and not end_of_video:  # CDN can't get the frame
            ...
            if not self.buffer_status:
                ...
                # not enough .let the client buffering
                    ...
                    self.buffer_size = 0
                    self.buffer_status = True  # buffer_status is True means that the buffer should be buffering and will not play videos

                # calculate the play time , the real time ,the latency
                # the normal play the frame
                if self.play_time_counter >= self.skip_time_frame:  # if the number of play frame is higher than 1000000000
                    ...
                    self.play_time_counter = int(self.play_time / FRAME_TIME_LEN)  # calculate the frame number

                #self.latency = (self.newest_frame - self.video_chunk_counter) * FRAME_TIME_LEN + self.buffer_size  # latency is the duration to play
                self.time = self.cdn_arrive_time[quality][self.video_chunk_counter]
            else:  # buffer is buffering, no frames will be played out
                rebuf = duration
                play_len = 0
                self.time = self.cdn_arrive_time[quality][self.video_chunk_counter]
            ...
            if self.Debug:
                self.log_file.write(str(self.time) + '\t '+ str(cdn_rebuf_time) + '\t' + str(rebuf) + '\t' + str(duration) + '\t' + str(video_frame_size) + '\t' + str(play_len) + '\t' + str(self.video_chunk_counter - 1)
                                    + '\t' + str(self.newest_frame) + '\t' + str(self.buffer_size) + '\t' + str(self.play_time) + '\t' + str(self.play_time_counter) + '\t' + str(self.latency) + '\t' +  "000\n")
                ...
                #                    "latency %.4f\t" % self.latency + "000\n")
            # Return the loop
            cdn_has_frame = []  # the function is not clear
            for bitrate in range(BITRATE_LEVELS):
        ...
        if int(self.time / 0.5) >= len(self.cooked_bw):  # why the time should divide 0.5????
            end_of_video = True
        else:
        ...
        if self.gop_flag[quality][self.video_chunk_counter + 1] == 1:  # the relationship between the video chunk counter and the frame index??? in my opinion, the video chunk counter is the frame index
            self.indicate_flag = self.indicate_flag ^ True
            if self.indicate_flag:
                self.decision = True
            self.next_I_frame = self.video_chunk_counter + 30 + 1
        # If the buffering
        if self.buffer_status and not end_of_video:  # the buffer is not up to the target buffer and the video is not the end of video
            # let the buffer_size to expand to the target_buffer
            if self.buffer_size < Target_buffer[target_buffer]:  # Target buffer is the size of the buffer, the value of it can be 2s or 3s. and the target buffer is the choice of different buffer length
            ...
            # if the buffer is enough
                ...
                rebuf = duration
            # calculate the play time , the real time ,the latency
            self.play_time_counter = int(
                self.play_time / FRAME_TIME_LEN)  # the function of play time counter is not clear
            #self.latency = (self.newest_frame - self.video_chunk_counter) * FRAME_TIME_LEN + self.buffer_size
            # Debug Info
            ...
            if self.Debug:
                self.log_file.write(str(self.time) + '\t ' + str(cdn_rebuf_time) + '\t' + str(rebuf) + '\t' + str(
                    duration) + '\t' + str(video_frame_size) + '\t' + str(0) + '\t' + str(
                    self.video_chunk_counter - 1)
                                    + '\t' + str(self.newest_frame) + '\t' + str(self.buffer_size) + '\t' + str(
                    self.play_time) + '\t' + str(self.play_time_counter) + '\t' + str(self.latency) + '\t' + "111\n")
                #self.log_file.write("real_time %.4f\t" % self.time +
                #                    "cdn_rebuf%.4f\t" % cdn_rebuf_time +
                #                    "client_rebuf %.3f\t" % rebuf +
                #                    "download_duration %.4f\t" % duration +
        ...
        # If not buffering
            ...
            self.buffer_size += self.gop_time_len[quality][self.video_chunk_counter]
            self.time += duration
            if self.latency > latency_threshold:  # need to further research????
                self.skip_time_frame = self.video_chunk_counter
                if self.newest_frame >= self.next_I_frame:
                    self.add_frame = self.next_I_frame - self.skip_time_frame - 1
                    self.video_chunk_counter = self.next_I_frame
            ...
            if self.Debug:
                self.log_file.write(str(self.time) + '\t ' + str(cdn_rebuf_time) + '\t' + str(rebuf) + '\t' + str(
                    duration) + '\t' + str(video_frame_size) + '\t' + str(duration * play_duration_weight) + '\t' + str(
                    self.video_chunk_counter - 1)
                                    + '\t' + str(self.newest_frame) + '\t' + str(self.buffer_size) + '\t' + str(
                    self.play_time) + '\t' + str(self.play_time_counter) + '\t' + str(self.latency) + '\t' + "222\n")
                #self.log_file.write("real_time %.4f\t" % self.time +
                #                    "cdn_rebuf%.4f\t" % cdn_rebuf_time +
                #                    "client_rebuf %.3f\t" % rebuf +
                #                    "download_duration %.4f\t" % duration +
