...
class ActorNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate):
        ...
        self.GRUcell = tf.contrib.rnn.GRUCell(num_units = 128)
        self.dense0 = tf.keras.layers.Dense(128, activation='relu')
        self.dense_out = tf.keras.layers.Dense(self.a_dim, activation='softmax')
    ...
    def create_actor_network(self):
        with tf.variable_scope('actor'):
            inputs_ = tflearn.input_data(shape=[None, self.s_dim[0], self.s_dim[1]])
            inputs = tf.transpose(inputs_, perm=[0, 2, 1])

            outputs, states = tf.nn.dynamic_rnn(cell=self.GRUcell, dtype=tf.float32, inputs = inputs)
            outputs1 = self.dense0(outputs[:, -1, :])
            out = self.dense_out(outputs1)
...
class CriticNetwork(object):
    ...
    def create_critic_network(self):
        with tf.variable_scope('critic'):
            ...
            outputs, states = tf.nn.dynamic_rnn(cell=self.GRUcell, dtype=tf.float32, inputs=inputs)
            #outputs1 = tflearn.fully_connected(outputs[:,-1,:], 128, activation='relu')
            outputs1 = self.dense0(outputs[:, -1, :])
            #out = tflearn.fully_connected(outputs1, self.a_dim, activation='softmax')
            out = self.dense_out(outputs1)
